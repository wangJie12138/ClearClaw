...
import sys
from clearclaw.core.provider import get_provider
...
def config_wizard():
    ...
    model_name = questionary.text(
        "输入指定的模型型号(如gpt-4o-mini, qwen-max, glm-4等):",
        style=clear_style
    ).ask()

    if model_name is None:
        console.print("[dim #8d52ff]!!! 录入中断，ClearClaw配置已取消。[/dim #8d52ff]")
        return
    ...
    if provider in ["openai", "anthropic"]:
        base_url = questionary.text(
            f"输入{provider}代理Base URL(直连请直接回车跳过):",
            style=clear_style
        ).ask()
    elif provider == "ollama":
        base_url = questionary.text(
            "输入Ollama Base URL(默认http://localhost:11434，直接回车跳过):",
            style=clear_style
        ).ask()
    else:
        base_url = questionary.text(
            "输入兼容Base URL (不填直接回车将使用官方默认地址):",
            style=clear_style
        ).ask()
    ...
    with Status(f"[bold #8d52ff]正在连接 {provider.upper()} 引擎并发送探测包...[/bold #8d52ff]", spinner="dots", spinner_style="#00ffff"):
        try:
            ...
            console.print(f"[bold #8d52ff][配置失败!][/bold #8d52ff]  无法连接到模型，请检查Key、Base URL、模型型号或网络！\n[dim]错误信息: {str(e)}[/dim]")
            return
    ...
    console.print(Panel(
        ...
        f"输入[bold #00ffff]clearclaw run[/bold #00ffff]即可启动系统！",
        border_style="#00ffff"
    ))

def _show_boot_error():
    console.print(Panel(
        "[bold #00ffff]ClearClaw未完成配置![/bold #00ffff]\n\n"
        "[#8d52ff]检测到API Key、模型或Baseurl。请重新执行以下命令完成配置：[/#8d52ff]\n"
        "[bold #00ffff]clearclaw config[/bold #00ffff]",
        title="[bold #8d52ff] Boot Sequence Failed[/bold #8d52ff]",
        border_style="#8d52ff"
    ))
...
def run_agent():
    ...
    import entry.main as clearclaw_main
    clearclaw_main.main()
...
def run_monitor():
    ...
    try:
        import entry.monitor as clearclaw_monitor
        clearclaw_monitor.main()
    except ImportError as e:
        console.print(f"[bold red]启动失败：找不到监视器模块！[/bold red]\n[dim]请确保monitor.py和cli.py在同一目录下。\n报错信息: {e}[/dim]")
