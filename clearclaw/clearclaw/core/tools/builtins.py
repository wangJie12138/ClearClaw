...
from .base import clearClaw_tool, ClearClawBaseTool
import os
import json
import uuid
import threading
...
def get_system_model_info() -> str:
    """
    获取当前ClearClaw正在运行的底层大模型（LLM）型号和提供商信息。
...
def save_user_profile(new_content: str) -> str:
    ...
    1.请先调用read_user_profile获取当前的完整档案。
    2.在你的上下文中，将新信息融入档案，并删去冲突或过时的旧信息。
    3.将修改后的一整篇完整Markdown文本作为new_content参数传入此工具。
...
def calculator(expression: str) -> str:
    ...
    注意：参数expression必须是一个合法的Python数学表达式字符串。
    """
    try:
...
def schedule_task(target_time: str, description: str, repeat: str = None, repeat_count: int = None) -> str:
    """
    为一个未来的任务设定闹钟或提醒。
    参数 target_time 必须是严格的格式："YYYY-MM-DD HH:MM:SS"（请先调用get_current_time获取当前时间，并在其基础上推算）。
    ...
    with tasks_lock:
        ...
        try:
            ...
            if len(new_tasks) == len(tasks):
                return f"删除失败：未找到ID为 {task_id} 的任务。"
...
def modify_scheduled_task(task_id: str, new_time: str = None, new_description: str = None) -> str:
    ...
    with tasks_lock:
        ...
        try:
            ...
            if not found:
                return f"修改失败：未找到ID为 {task_id} 的任务。"
