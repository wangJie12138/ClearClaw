...
class DynamicSkillInput(BaseModel):
    ...
    command: Optional[str] = Field(
        default="",
        description="仅在mode='run'时需要。你要执行的完整命令，保留{baseDir}占位符。"
    )
        ...
        def lazy_runner(mode: str, command: str = "") -> str:
            ...
            if mode == "help":
                ...
                return (
                    ...
                    f"====================================\n"
                    f"提示：请根据以上说明，如果觉得能解决问题，就将mode设为 'run'，"
                    f"并将拼装好的执行命令填入command重新调用。"
                )
            elif mode == "run":
                if not command:
                    return "错误：在'run'模式下，必须提供command参数！"
                
                actual_cmd = command.replace("{baseDir}", f"skills/{skill_info['folder']}")
                return execute_office_shell.invoke({"command": actual_cmd})
            else:
                return "错误：mode参数只能是'help'或'run'。"
...
def load_dynamic_skills(force_rescan: bool = False) -> List[StructuredTool]:
    ...
    Args:
        force_rescan: 是否强制重新扫描技能目录（默认False）
