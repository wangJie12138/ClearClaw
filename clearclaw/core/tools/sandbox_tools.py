...
def _get_safe_path(relative_path: str) -> str:
    ...
    """
    # 将OFFICE_DIR转化为标准绝对路径
    base_dir = os.path.abspath(OFFICE_DIR)
    # 将目标路径转化为绝对路径
    target_path = os.path.abspath(os.path.join(base_dir, relative_path))
    
    # 核心防御：目标路径必须以OFFICE_DIR开头！
    if not target_path.startswith(base_dir):
        raise PermissionError(f"越权拦截：你试图访问沙盒外的路径 '{relative_path}'！你只能在office工位内活动。")
...
def list_office_files(sub_dir: str = "") -> str:
    """
    查看你的office工位里有哪些文件和文件夹。
    如果sub_dir为空，则查看工位根目录。
    """
    try:
        ...
        if not items:
            return f"[{sub_dir if sub_dir else 'office根目录'}]是空的。"
...
def read_office_file(filepath: str) -> str:
    """
    读取office工位里指定文件的内容。
    filepath参数应该是相对于office的路径，例如 "test.py" 或 "skills/my_skill.py"。
    """
    try:
...
def write_office_file(filepath: str, content: str, mode: str = "w") -> str:
    ...
    2. 如果你需要重命名文件或删除文件，请直接使用execute_office_shell工具执行`mv`或`rm`命令。
...
def execute_office_shell(command: str) -> str:
    ...
    【极其重要的环境限制】：
    1. 跨平台注意：当前宿主机可能是 Windows、Linux 或 Mac。请根据你得到的环境反馈，使用对应的原生Shell命令（例如Win用 dir/del，Linux 用 ls/rm）。如果命令报错，请自行调整重试！
    2. 这是一个非交互式终端！所有命令必须携带免确认参数（如 -y, --quiet）。
    3. 禁止使用cd命令跳出当前目录，你的活动范围仅限office。
    ...
    try:
        ...
        for pattern in dangerous_patterns:
            if re.search(pattern, command):
                return f"!!!权限拒绝：检测到危险的目录跳转指令。你被禁止离开office工位！"
