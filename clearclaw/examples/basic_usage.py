...
def main():
    ...
    print("初始化 ClearClaw 核心引擎...")
    
    app = create_agent_app(provider_name="aliyun", model_name="glm-5")
    
    print("ClearClaw 启动完毕！你可以开始提问了。(输入 'quit' 或 'q' 退出)")
    print("-" * 50)
            ...
            for node_name, node_state in event.items():
                ...
                if node_name == "agent":
                    ...
                    if latest_message.tool_calls:
                        ...
                        print(f"决策: 准备调用工具 [{tool_call['name']}]")
                        print(f"参数: {tool_call['args']}")
                    elif latest_message.content:
                        # 如果没有调用工具，且有文本内容，说明大模型给出了最终回答
                        print(f"\n [ClearClaw]: {latest_message.content}")
